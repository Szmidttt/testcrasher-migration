...
// extern "C" NS_EXPORT void ThrowException()  {
// #ifdef MOZ_PHC
...
// Helper function for Rust to get function address from Win64 CFI test map
#if XP_WIN && HAVE_64BIT_BUILD && defined(_M_X64) && !defined(__MINGW32__)
extern "C" NS_EXPORT uint64_t GetWin64CFITestMap_GetFunctionAddress(int16_t fnid) {
  auto m = GetWin64CFITestMap();
  if (m.find(fnid) == m.end()) {
    return 0;
  }
  return (uint64_t)m[fnid];
}
#endif
